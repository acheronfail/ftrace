...
const tableFile = await fs.readFile(`${args['linux-source']}/${SYSCALL_TABLE}`, 'utf-8');
const definitions: SyscallDefinition[] = await Promise.all(
  tableFile
    .split(/\n/)
    ...
    .map(async ({ id, abi, name, implName }) => {
      ...
      if (DEPRECATED_SYSCALLS.includes(name)) {
        return { id, abi, name, implName, skipped: 'deprecated' };
      }
      ...
      const { parameterless, results } = await findSyscall(args['linux-source'], name);
      if (parameterless) {
        return { id, abi, name, skipped: 'parameterless' };
      }
      ...
      const matches = (
        await Promise.all(
          results
            ...
            .map((rgMatch) => {
              // NOTE: these casts should be safe since the linux source is utf-8
              return {
                path: (rgMatch.data.path as RgText).text.replace(/^\.\//, ''),
